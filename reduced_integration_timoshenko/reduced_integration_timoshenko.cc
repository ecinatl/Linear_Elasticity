...
ElasticProblem<dim>::ElasticProblem (int fe_degree)
  :
    ...
    fe (FESystem<dim>(FE_Q<dim>(fe_degree), dim))
{}
...
void ElasticProblem<dim>::setup_system ()
{
  ...
  //BEGIN added b/c that's what timo did
  ...
  //END added b/c that's what timo did
...
//  std::vector<double> w = quadrature_formula.get_weights();
...
//  std::cout<<std::endl;
  ...
  QGauss<dim-1> face_quadrature_formula(2*fe_degree+2);
...
//      std::vector<double> w = gauss_2.get_weights();
//      for(int i = 0; i < w.size(); i++){
//           std::cout<<w[i]<<std::endl;
...
//END boundary integrals
      ...
      hanging_node_constraints.distribute_local_to_global(cell_matrix,
                                                          ...
                                                          local_dof_indices,
                                                          system_matrix,
                                                          system_rhs);//added b/c that's what Timo did
...
//output the displacement in the bottom right corner (timo thinks this is where the max displacement will be)
  ...
  VectorTools::point_value (dof_handler, solution,
   ...
   std::cout << "  y-displacement_at_(8.,0.): "
             << val(1);
 // std::cout << "  norm_displacement_at_(8.,0.): "
   //         << sqrt(val(0)*val(0)+val(1)*val(1));  //more digits "std cout double more digits"
  ...
  for(unsigned int it = 0; it < 1; it++){
      ...
      if (it == 0){ //direct solver
          ...
          for(int r =4; r<5; r++){
             ...
             //double lambdas[3]={100, 50000.0, 500000.0};

             //for (unsigned int i=0; i < 3;i++)
            // {
                //  lamb = lambdas[i];
                //  nu = 0.5*lamb/(lamb+mu_scalar);
                  std::cout<<std::endl<<"fe_degree="<<fe_degree<<", red_quad="<<red_quad<<", lambda="<<lamb<<", mu="<<mu_scalar<<", nu="<<nu<<std::endl;
                  old_main(fe_degree);
          //   }
